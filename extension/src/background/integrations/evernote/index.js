    ...
    return this.storage
      ...
      .then(accessToken => {
        ...
        return enhancedFetch(
          `${REST_BASE_URL}/evernote/authorize-url?redirect_url=${encodeURIComponent(redirectUrl)}`
        ).then(data => {
          ...
          return browser.identity
            ...
            .then(url => {
              ...
              return enhancedFetch(
                `${REST_BASE_URL}/evernote/access-token`,
                {
                  method: 'POST',
  ...
  sendNotes() {
    return this.getAccessToken()
      .then(token => {
        ...
        return store.listNotebooks().then(notebooks => {
          const yinotebook = notebooks.find(notebook => notebook.name === Service.YI_NOTEBOOK_NAME)
          if (yinotebook) {
            return yinotebook;
          }
        ...
        .then(notebook => {
          ...
          return store.createNote(notePayload)
            .then(note => {
              return this.storage
                .set(this.data.id, note.guid)
                .then(() => note);
            });
        });
      })
